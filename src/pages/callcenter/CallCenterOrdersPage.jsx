  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-800 text-xs font-bold mb-2 border border-amber-500/20">
            <Headphones className="w-3.5 h-3.5" />
            <span>Call Center & Operator Moduli (RBAC)</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900">Buyurtmalar Oqimi & Boshqaruvi</h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Kelib tushgan yangi buyurtmalarni ko'rib chiqing, mijozlar bilan bog'laning va statusni yangilang.
          </p>
        </div>

        {/* Live Pending Pulse Badge */}
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-amber-50 border border-amber-200">
          ...
          <div className="text-xs">
            <span className="text-slate-600 font-medium">Tasdiqlash kutilmoqda: </span>
            <strong className="text-amber-800 font-black text-sm">{counts.Pending} ta buyurtma</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
        {[
          { key: 'all', label: 'Barcha buyurtmalar', count: counts.all, color: 'text-slate-800', bg: 'bg-white' },
          { key: 'Pending', label: 'Kutilmoqda', count: counts.Pending, color: 'text-amber-700', bg: 'bg-amber-50/70 border-amber-200' },
          { key: 'Confirmed', label: 'Tasdiqlandi', count: counts.Confirmed, color: 'text-blue-700', bg: 'bg-blue-50/70 border-blue-200' },
          { key: 'Shipped', label: "Yo'lda", count: counts.Shipped, color: 'text-purple-700', bg: 'bg-purple-50/70 border-purple-200' },
          { key: 'Delivered', label: 'Yetkazildi', count: counts.Delivered, color: 'text-emerald-700', bg: 'bg-emerald-50/70 border-emerald-200' },
          { key: 'Cancelled', label: 'Bekor qilindi', count: counts.Cancelled, color: 'text-rose-700', bg: 'bg-rose-50/70 border-rose-200' },
        ].map((item) => (
          <button
            key={item.key}
            ...
            className={`p-3.5 rounded-2xl border text-left transition ${
              statusFilter === item.key
                ? 'ring-2 ring-slate-900 shadow-md ' + item.bg
                : 'hover:border-slate-300 ' + item.bg
            }`}
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-6 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="text-xs text-slate-500 font-medium self-end sm:self-center">
          Topilgan buyurtmalar: <strong className="text-slate-800">{filteredOrders.length} ta</strong>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white rounded-3xl border border-slate-200 p-8">
          ...
          <h3 className="text-base font-bold text-slate-900">Buyurtmalar ro'yxatini yuklab bo'lmadi</h3>
        </div>
      ) : filteredOrders.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
          <Package className="w-12 h-12 text-slate-300 mx-auto mb-3" />
          <h3 className="text-base font-bold text-slate-800">Ushbu parametrlar bo'yicha buyurtma topilmadi</h3>
          <p className="text-xs text-slate-500 mt-1">Status yoki qidiruv so'zini tozalab ko'ring.</p>
        </div>
      ) : (
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-slate-50/80 transition">
                      ...
                      <td className="py-4 px-4">
                        ...
                        <span className="text-[11px] text-slate-400 block mt-0.5">{formatDate(order.createdAt)}</span>
                      </td>

                      {/* Customer Info */}
                      <td className="py-4 px-4">
                        <span className="font-bold text-slate-900 block">{order.customerInfo?.name}</span>
                        <a
                          href={`tel:${order.customerInfo?.phone}`}
                          className="inline-flex items-center gap-1 text-brand-600 hover:text-brand-700 font-medium text-[11px] mt-0.5 hover:underline"
                        >
                      ...
                      <td className="py-4 px-4">
                        <div className="space-y-1 max-w-[200px]">
                          {order.items?.map((item, idx) => (
                            <div key={idx} className="truncate text-slate-700 font-medium text-[11px]">
                      ...
                      {/* Amount */}
                      <td className="py-4 px-4">
                        <span className="font-black text-slate-900 block">{formatCurrency(order.totalAmount)}</span>
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {order.status === 'Pending' && (
                            <>
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-[11px] transition shadow-sm"
                              ...
                              <button
                                ...
                                className="px-2 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-[11px] transition border border-rose-200"
                                title="Bekor qilish"
                              >
                                Bekor
                              </button>
                          ...
                          {order.status === 'Confirmed' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-[11px] transition shadow-sm"
                            >
                          ...
                          {order.status === 'Shipped' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] transition shadow-sm"
                            >
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-100 z-10 animate-slide-up max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
              <div>
                <h3 className="text-lg font-black text-slate-900">Buyurtma #{selectedOrder.id} Tafsilotlari</h3>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2 text-xs mb-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-slate-900">{selectedOrder.customerInfo?.name}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500">Telefon:</span>
                <a
                  href={`tel:${selectedOrder.customerInfo?.phone}`}
                  className="font-bold text-brand-600 flex items-center gap-1 hover:underline"
                >
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-slate-800 text-right">{selectedOrder.customerInfo?.address}</span>
              </div>
              <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                <span className="text-slate-500">To'lov:</span>
                <span className="font-bold text-slate-900">{selectedOrder.paymentMethod} • {formatCurrency(selectedOrder.totalAmount)}</span>
              </div>
            </div>

            {/* Items */}
            <div className="mb-4">
              <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Buyurtma tarkibi:</h4>
              <div className="space-y-2">
                {selectedOrder.items?.map((item, i) => (
                  <div key={i} className="flex items-center gap-3 p-2 rounded-xl border border-slate-100">
                    ...
                    <span className="text-xs font-extrabold">{formatCurrency(item.quantity * item.price)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="p-4 bg-amber-50/50 rounded-2xl border border-amber-200 mb-4">
              <h4 className="text-xs font-bold text-amber-900 mb-2">Buyurtma Statusini O'zgartirish:</h4>
            ...
            {selectedOrder.history && selectedOrder.history.length > 0 && (
              <div>
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Tarix va Harakatlar:</h4>
                  ...
                  {selectedOrder.history.map((h, idx) => (
                    <div key={idx} className="flex items-start gap-2 p-2 rounded-lg bg-slate-50 border border-slate-100">
                      ...
                      <div className="flex-1">
                        ...
                        <span>{h.note}</span>
                        <span className="text-[10px] text-slate-400 block mt-0.5">{formatDate(h.date)}</span>
                      </div>
                    </div>
                  ))}
                </div>
