...
export const ProductCard = ({ product, onSelect }) => {
  ...
  return (
    <div
      ...
      className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 shadow-xs hover:shadow-[0_12px_28px_-6px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_12px_28px_-6px_rgba(0,0,0,0.5)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-pointer relative p-2.5 sm:p-3"
    >
      {/* Top Image Showcase with Badges & Wishlist Heart */}
      <div className="relative w-full aspect-[3/4] bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden mb-2">
        ...
        <button
          ...
          className="absolute top-2 right-2 z-10 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/85 dark:bg-slate-900/85 hover:bg-white dark:hover:bg-slate-800 backdrop-blur-md border border-white/60 dark:border-slate-700 flex items-center justify-center transition-all duration-200 shadow-xs hover:scale-110 active:scale-90"
          title={isLiked ? t('product.removeFromWishlist') : t('product.addToWishlist')}
        >
          <Heart
            className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-colors ${
              isLiked ? 'fill-rose-500 text-rose-500' : 'text-slate-400 dark:text-slate-400 hover:text-rose-500'
            }`}
          />
        </button>
        ...
        <div className="absolute bottom-1.5 left-1.5 px-1.5 py-0.5 rounded-md bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200/80 dark:border-slate-700 text-[10px] font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1 shadow-xs">
          <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
          <span>{product.rating || 4.9}</span>
          <span className="text-slate-400 dark:text-slate-500 font-normal">({product.salesCount || 14})</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-xs sm:text-sm font-medium text-slate-800 dark:text-slate-100 line-clamp-2 leading-snug group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition min-h-[2rem] sm:min-h-[2.5rem]">
            {product.title}
          </h3>

          {/* Uzum Market Signature Monthly Installment Pill */}
          <div className="mt-1 mb-2">
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-amber-100/90 dark:bg-amber-950/60 text-amber-950 dark:text-amber-300 border border-amber-200/70 dark:border-amber-800/60 text-[10px] sm:text-[11px] font-bold leading-none">
        ...
        {/* Price & Action Row */}
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-end justify-between gap-1.5">
          <div className="min-w-0 flex-1">
            {hasDiscount && (
              <span className="text-[10px] sm:text-[11px] text-slate-400 dark:text-slate-500 line-through leading-none block">
            ...
            <span className="text-[9px] sm:text-[10px] text-slate-400 dark:text-slate-500 font-medium block truncate max-w-[120px] sm:max-w-[140px]">
          ...
          <button
            ...
            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center transition-all duration-200 flex-shrink-0 cursor-pointer ${
              isOutOfStock
                ...
                : inCart
                ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/30 ring-2 ring-emerald-200 dark:ring-emerald-900 font-bold'
                : 'bg-white dark:bg-slate-800 hover:bg-emerald-600 dark:hover:bg-emerald-600 text-slate-700 dark:text-slate-200 hover:text-white dark:hover:text-white border border-slate-200 dark:border-slate-700 hover:border-emerald-600 dark:hover:border-emerald-600 shadow-xs hover:scale-105 active:scale-95'
            }`}
            title={
              isOutOfStock
